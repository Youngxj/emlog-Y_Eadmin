	...
	$.fn.Huploadify = function(opts){
		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					var inputStr = '<input id="select_btn_'+instanceNumber+'" class="selectbtn" style="" type="file" name="fileselect[]"';
						...
						for(var i=0,len=files.length;i<len;i++){
							...
							if(parseInt(F.formatFileSize(f.size,true))<=option.fileSizeLimit){
								if($.inArray('*',typeArray)>=0 || $.inArray(f.name.split('.').pop(),typeArray)>=0){
									...
									EmlogMsgNotify('warning','','文件 "'+f.name+'" 类型不允许！','top right');
								}
							}
							else{
								EmlogMsgNotify('warning','','文件 "'+f.name+'" 大小超出限制！','top right');
								continue;
							}
						}
					}
