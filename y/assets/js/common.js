...
function checkform(){
	...
	if (t==""){
		EmlogMsgNotify('warning','','标题不能为空','top right');
		$("#title").focus();
		return false;
	}else if(0 == isalias(a)){
		return true;
	}else {
		EmlogMsgNotify('warning','','链接别名错误','top right');
		$("#alias").focus();
		return false
	};
}
...
function autosave(act){
	...
	if(alias != '') {
		if (0 != isalias(alias)){
			EmlogMsgNotify('error','','链接别名错误，自动保存失败','top right');
			//$("#msg").html("<span class=\"msg_autosave_error\"></span>");
	...
	EmlogMsgNotify('info','','正在保存……','top right');
	...
	$.post(url, querystr, function(data){
		...
		if(isrespone.test(data)){
			...
			EmlogMsgNotify('success','','成功保存于'+hours+':'+mins+':'+secs,'top right');
			...
			$("#savedf").attr("disabled", false);
			$("#savedf").val(btname);
			EmlogMsgNotify('error','','网络或系统出现异常...保存可能失败','top right');
